...
//     a real template effectively impossible.
...
// stay valid.
export function encodePlaceholders(source: string, spans: Span[]): Placeholdered {
  ...
  // what makes invariant 2 hold.
  const width = Math.max(MIN_COUNTER_WIDTH, Math.max(0, spans.length - 1).toString(36).length);
  ...
  return { placeholdered, spans: spanMap };
}
...
export function substitutePlaceholders(formatted: Doc, spans: Record<string, string>): Doc {
  ...
  return utils.mapDoc(formatted, (current) =>
    typeof current === "string" ? current.replace(pattern, (id) => spans[id] ?? id) : current,
  );
}
